module GCM
  class << self
    ...
    def encrypt(key, iv, aad, pt)
      ...
      # Plaintext
      each_message_block(pt, key, iv) do |block, ctr_block|
    ...
    def decrypt(key, iv, aad, ct)
      ...
      # Plaintext
      each_message_block(ct, key, iv) do |block, ctr_block|
      ...
      final_block = (aad.bytesize*8) * (2**64) | ct.bytesize*8
      tag = mul(h, tag^final_block)
      tag ^= iv_block(key, iv)
    ...
    private def each_message_block(message, key, iv)
      j = 0
